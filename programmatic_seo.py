...
================================================================================
Pipeline:
  1. Generate keyword matrix from Nonbank's country/crypto/use-case data (free)
...
# Countries where Nonbank CAN issue cards (human-readable names + lang)
COUNTRIES = {
    # Tier 1 revenue
...
def check_serp_competition(keyword: str, serpapi_key: str, lang: str = "en", country: str = "") -> dict:
    ...
    forum_count = 0
    weak_count = 0
    nonbank_present = False

    for r in organic[:10]:
        link = r.get("link", "")
        domain = r.get("displayed_link", "").split("/")[0].replace("www.", "").lower() if r.get("displayed_link") else ""

        if any(d in link for d in ["nonbank.io", "nonbank.io"]):
            nonbank_present = True
    ...
    return {
        ...
        "nonbank_present": nonbank_present,
...
def generate_page_specs(scored_keywords: List[dict], min_opportunity: str = "medium") -> List[PageSpec]:
    ...
    for kw in scored_keywords:
        ...
        if template == "country_card":
            title = f"Crypto Visa Card in {country_name} — Spend USDT, BTC & ETH | Nonbank"
            ...
            title = f"How to Spend Crypto in {country_name} — USDT & Bitcoin Guide 2026 | Nonbank"
            meta = f"Complete guide to spending cryptocurrency in {country_name}. Convert USDT or Bitcoin to {cdetails.get('currency_name', 'local currency')} with Nonbank Visa card. Low fees, instant conversion."
...
<head>
    ...
    <link rel="canonical" href="https://nonbank.io/{slug}">
    <meta property="og:title" content="{title}">
    <meta property="og:description" content="{meta_description}">
    <meta property="og:type" content="website">
    <meta property="og:url" content="https://nonbank.io/{slug}">
    ...
    {{
        ...
        "url": "https://nonbank.io/{slug}",
        "category": "Cryptocurrency Card",
        "areaServed": "{country_name}",
        "availableChannel": {{
            "@type": "ServiceChannel",
            "serviceUrl": "https://nonbank.io"
        }}
    }}
    </script>
</head>
<body>
    <h1>{h1}</h1>
    {content_blocks}
    <p><a href="https://nonbank.io">Get your Nonbank Crypto Visa Card →</a></p>
</body>
</html>
"""
...
def _default_content_blocks(spec: PageSpec) -> str:
    ...
    <section>
        ...
        Nonbank converts your USDT, Bitcoin, or Ethereum to {currency} instantly at the point of sale —
        no manual exchange, no bank transfers, no delays.</p>
        <h3>How It Works</h3>
        <ol>
            <li>Load your Nonbank wallet with USDT (TRC-20), BTC, or ETH</li>
            <li>Your Nonbank Visa card is linked to your wallet balance</li>
        ...
        <p>Nonbank cards are fully supported in {country}. Apply through the Nonbank app
    ...
    <section>
        ...
        <p>A crypto Visa card like Nonbank lets you spend directly at any of the 60M+

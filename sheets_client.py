"""
sheets_client.py — Google Sheets integration for Nonbank SEO Agent
================================================================
...
def push_audit_results(creds_json: str, results: list[dict], sheet_name: str = "GEO Audit") -> int:
    ...
    try:
        ...
        ws = spreadsheet.add_worksheet(title=sheet_name, rows=1000, cols=10)
        ws.update("A1:H1", [["Date", "Query", "Nonbank Visible", "Nonbank Position", "Top Result", "Competitors", "AI Overview", "Total Results"]])
    ...
    for r in results:
        ...
        if r.get("ai_overview"):
            ai_mention = "Nonbank mentioned" if r["ai_overview"].get("nonbank_mentioned") else "No Nonbank"
        new_rows.append([
            today,
            r.get("query", ""),
            "Yes" if r.get("nonbank_visible") else "No",
            r.get("nonbank_position") or "",

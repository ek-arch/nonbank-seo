# Nonbank Website Visibility Report
...
Comprehensive technical SEO audit and fixes for nonbank.io (redirects from nonbank.io). Work covered Webflow on-site SEO, Google Search Console submissions, Cloudflare investigation, and LLMs.txt for GEO (Generative Engine Optimization).
...
**Additional fix:** Old Home page and Test Page canonicals set to `https://nonbank.io` via Webflow Editor to redirect any residual SEO value to homepage.
...
- 5 use cases
- Comparison table (Nonbank vs Crypto.com vs Binance Card vs Revolut)
- Full page list with URLs

**Deployed:** Uploaded to Webflow Dashboard (custom file hosting). Live at `https://nonbank.io/llms.txt`.

**Verified live:** Yes — WebFetch confirmed accessible and correct content.

### 1.5 Sitemap — RESUBMITTED (issue persists)
**Problem:** Google Search Console shows "Couldn't fetch" for `https://nonbank.io/sitemap.xml`. Status: Unknown, 0 discovered pages.
...
**Attempted fix via Cloudflare Worker:** A `nonbank-proxy` Worker already exists in the Cloudflare account (ek@nonbank.io) with sitemap proxy code that serves correct `application/xml` headers. However:
- nonbank.io and nonbank.io DNS zones are NOT in this Cloudflare account (no domains found)
...
**Worker endpoint verified working:** `https://nonbank-proxy.ek-3ff.workers.dev/sitemap.xml` returns correct `application/xml` content-type. Just needs a route.
...
- Resubmitted `https://nonbank.io/sitemap.xml` on Apr 7, 2026
...
| Page | Status |
|------|--------|
| `https://nonbank.io/` | Indexing requested |
| `https://nonbank.io/exchanger` | Indexing requested |
| `https://nonbank.io/buy-crypto` | Indexing requested |
| `https://nonbank.io/countries` | Indexing requested |
| `https://nonbank.io/cryptowallet` | Indexing requested |
| `https://nonbank.io/for-business/business` | Indexing requested |
| `https://nonbank.io/blog` | Indexing requested |
...
| Task | Priority | Notes |
|------|----------|-------|
| **Fix sitemap MIME type** | Medium | Requires access to the Cloudflare account managing nonbank.io DNS, OR adding nonbank.io as zone to current CF account to attach Worker route |
...
- **nonbank.io** → 301 redirects to **nonbank.io** (all paths)
- Both domains use Cloudflare nameservers but zones are NOT in the `ek@nonbank.io` Cloudflare account
- **nonbank.io** is hosted on Webflow (confirmed by `x-wf-region: us-east-1` header)
- Cloudflare Worker `nonbank-proxy` exists at `nonbank-proxy.ek-3ff.workers.dev` — handles CORS proxy for Hex.tech/Collaborator.pro APIs + sitemap MIME type fix, but has no routes attached to nonbank.io
- Webflow Editor access: `https://nonbank.io?edit=1` (requires invitation)
...
```bash
# Check hreflang tags
curl -s https://nonbank.io/ | grep -i hreflang

# Check canonical
curl -s https://nonbank.io/ | grep -i canonical

# Check LLMs.txt
curl -s https://nonbank.io/llms.txt | head -20

# Check sitemap content-type (the problem)
curl -sI https://nonbank.io/sitemap.xml | grep content-type
# Returns: application/rss+xml (WRONG — should be application/xml)

# Check Worker sitemap fix (correct but not routed)
curl -sI https://nonbank-proxy.ek-3ff.workers.dev/sitemap.xml | grep content-type
# Returns: application/xml (CORRECT)

# Check DNS
dig +short NS nonbank.io

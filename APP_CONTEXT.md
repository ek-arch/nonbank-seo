# Nonbank SEO & GEO Intelligence Agent — App Context

**URL:** https://nonbank-seo.streamlit.app
...
| `geo_visibility.py` | ~200 | SerpAPI-powered GEO visibility tracker. 15 default queries (branded, generic, geo-targeted). Checks Nonbank presence + competitor detection in SERPs. |
...
| `templates/base.html` | — | Responsive HTML template for programmatic SEO pages (nav, breadcrumbs, hero, content, FAQ accordion, CTA, related pages, footer). Nonbank brand CSS. |
| `worker/nonbank-proxy.js` | ~140 | Cloudflare Worker: `/crypto-card/*` (KV pages), `/sitemap.xml` (merged sitemap), `/hex/*`, `/collaborator/*` |
...
| Stages | Nonbank Metrics | `page_nonbank_metrics` | Platform stats (pre/post cashback), P&L, country breakdown, revenue segments |
...
| Growth | GEO Tracker | `page_geo_tracker` | Semrush-style AI visibility monitor: discover prompts via Claude → monitor Nonbank mentions in Perplexity answers → track competitors & sources → find opportunities |
...
| Cloudflare Worker | Deployed | nonbank-proxy.ek-3ff.workers.dev (CORS proxy + programmatic SEO pages) |
...
4. Market existence (0-0.15) — Nonbank operates in this market
...
**Goal:** Track whether Nonbank appears in AI-generated answers when users ask about crypto cards.

**Pipeline:** Claude discovers realistic prompts (8 categories × 15 markets × 5 languages) → Perplexity monitors each prompt → Tracks Nonbank mention, competitor brands, source count → Identifies opportunities (prompts where competitors appear but Nonbank doesn't)
...
## Cloudflare Worker (nonbank-proxy)

**Account:** 3ff8191356080bd0d901586d6098dcde
**URL:** nonbank-proxy.ek-3ff.workers.dev
**Routes:**

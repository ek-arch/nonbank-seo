"""
llm_client.py — Anthropic Claude API wrapper for Nonbank SEO & GEO Agent
======================================================================
...
6. Section: Practical option — introduce Nonbank naturally, explain use case (not hype),
   mention nonbank.io 1-2 times
...
- Do NOT oversell Nonbank. Do NOT claim "best" without comparison.
- Position it as "one of the simple/practical options"
- Journalistic, third-person style ("Nonbank offers..." not "We offer...")

=== NONBANK FACTS (use relevant ones) ===
- Non-custodial — user keeps control of keys at all times
- Unified portfolio: connects banks, exchanges, hardware wallets, DeFi wallets in one app
- Virtual + physical Visa cards funded from self-custody
- Hardware wallet support (Ledger / Trezor)
- NON ID — DeFi identity with gamified rewards ("Nons")
- AML screening blocks sanctioned-wallet transfers automatically
- Web3 + Web2 consolidation (no switching between apps)
- Website: nonbank.io

=== COMPETITORS (mention fairly) ===
- Custodial cards: Crypto.com Card, Coinbase Card, Binance Card, Wirex, Revolut
- Non-custodial / self-custody: MetaMask Card, Gnosis Pay, Ledger Card, Zengo
...
Rules:
- Preserve ALL proper nouns: Nonbank, USDT, USDC, BTC, ETH, Visa, Ledger, Trezor, MetaMask, Gnosis Pay.
...
_SYSTEM_RECOMMEND = """\
You are a data-driven SEO & GEO strategist for Nonbank (nonbank.io), a non-custodial
wallet with Visa card and unified portfolio manager (banks + exchanges + hardware wallets).
...
- GEO: Recommend which articles to restructure for AI citability (FAQ sections,
  question headers, stat-dense paragraphs). Identify 3-5 AI queries where Nonbank
...
def revise_press_release(
    ...
    *,
    ...
    system = (
        "You are a crypto fintech PR editor for Nonbank (nonbank.io) — a non-custodial wallet + Visa card + unified portfolio app. "
        ...
        "Preserve proper nouns: Nonbank, USDT, USDC, BTC, ETH, Visa, Ledger, Trezor, MetaMask, Gnosis Pay."
    )
    user_msg = (
        f"## Current Draft\n\n{current_draft}\n\n---\n\n"
...
2. RELEVANCE CHECK — If Nonbank does NOT fit the specific request (e.g.
   they specifically want a custodial/exchange card and Nonbank is non-custodial),
   either skip mentioning Nonbank entirely or be honest about the fit.
...
7. BRAND MENTION — mention Nonbank ONLY if it genuinely fits the question.
   Do NOT promote aggressively. If Nonbank doesn't match what the person
   needs, recommend other products instead and skip Nonbank entirely.
   When mentioning Nonbank, keep it vague and general — just say "nonbank"
   or "nonbank wallet/card". Do NOT over-list features or technical details.
   Treat it like a wallet/card you've been using. Example: "been using
   nonbank lately, ok for pulling self-custody funds to a card without
   giving up keys"
8. ACCURACY — do not hallucinate facts. If unsure, keep it general.
   Do NOT claim Nonbank has features it doesn't have.

PLATFORM TONE:
- Reddit: slightly skeptical, honest, grounded, no hype
- Twitter/X: short, punchy, slight attitude, degen CT tone, lowercase,
  abbreviations, slang. Like "been using nonbank for a while now, works
...
def generate_comment_reply(
    ...
    *,
    ...
    user_msg += (
        ...
        "Be genuinely helpful first — mentioning Nonbank should feel incidental."
    )
    resp = _call_with_retry(
        _client(api_key),
        model=model,

...
- **Untapped bonus** (+10 pts) — Nonbank doesn't rank yet
...
""")
        ...
        if "pipe_ac_results" in st.session_state and st.session_state.get("pipe_ac_results"):
            ...
            if not pplx_key:
                ...
                if max_ai > 0 and st.button("🤖 Check AI Visibility", type="primary", key="pipe_ai"):
                    progress = st.progress(0)
                    ai_results = []
                ...
                if "pipe_ai_results" in st.session_state:
                    ...
                    visible = sum(1 for r in ai_results if r.get("nonbank_visible"))
                    st.metric("Nonbank Visible", f"{visible}/{len(ai_results)}")
                    ai_df = pd.DataFrame([{
                        "keyword": r["keyword"], "nonbank_visible": r.get("nonbank_visible", False),
    ...
    with tab_geo_audit:
        ...
        if not pplx_key:
            ...
            if "geo_market_results" in st.session_state:
                ...
                total = len(results)
                nonbank_total = sum(1 for r in results if r.get("nonbank_visible"))
                errors = sum(1 for r in results if r.get("error"))

                col_a, col_b, col_c = st.columns(3)
                col_a.metric("Markets Tested", len(by_market))
                col_b.metric("Total Prompts", total)
                col_c.metric("Nonbank Visible Overall", f"{nonbank_total}/{total - errors}")

                st.subheader("Per-Market Results")
                sorted_markets = sorted(by_market.values(), key=lambda x: x["nonbank_pct"])
                for ms in sorted_markets:
                    nonbank_pct = ms["nonbank_pct"]
                    icon = "🔴" if nonbank_pct == 0 else ("🟡" if nonbank_pct < 50 else "🟢")
                    comps_str = ", ".join(f"{c[0]} ({c[1]}x)" for c in ms["top_competitors"]) or "None"
                    with st.expander(f"{icon} **{ms['market_name']}** — {ms['nonbank_visible']}/{ms['prompts_tested']} ({nonbank_pct}%) | {comps_str}"):
                        for r in ms["results"]:
                            vis = "✅" if r.get("nonbank_visible") else "❌"
                ...
                table_rows = [{"Market": ms["market_name"],
                    "Nonbank Visible": f"{ms['nonbank_visible']}/{ms['prompts_tested']}", "Nonbank %": ms["nonbank_pct"],
                    "Top Competitor": ms["top_competitors"][0][0] if ms["top_competitors"] else "—",
                    "Status": "🔴 Not visible" if ms["nonbank_pct"] == 0 else ("🟡 Partial" if ms["nonbank_pct"] < 50 else "🟢 Good"),
                } for ms in sorted_markets]
                st.dataframe(pd.DataFrame(table_rows), use_container_width=True, hide_index=True)

                export_rows = [{"market": r.get("market", ""), "language": r.get("language", ""),
                    "category": r.get("category", ""), "prompt": r.get("prompt", ""),
                    "nonbank_visible": r.get("nonbank_visible", False),
...
1. **Text mention** — does Perplexity mention Nonbank?
2. **Source citations** — does Perplexity cite nonbank.io?
...
""")
        ...
        if not pplx_key:
            ...
            if "ai_audit_results" in st.session_state:
                ...
                col_b.metric("Nonbank in Text", f"{summary['nonbank_in_text_count']} ({summary['nonbank_in_text_pct']}%)")
                col_c.metric("Nonbank in Citations", f"{summary['nonbank_in_citations_count']} ({summary['nonbank_in_citations_pct']}%)")
                col_d.metric("Est. Cost", f"${summary['estimated_cost_usd']:.3f}")

                if summary["nonbank_visible_pct"] == 0:
                    st.error(f"⚠️ Nonbank NOT visible. Competitors: {', '.join(c[0] for c in summary['top_competitors_in_ai'][:3])}")
                elif summary["nonbank_visible_pct"] < 30:
                    st.warning(f"Nonbank visible in {summary['nonbank_visible_pct']}% of answers")
                else:
                    st.success(f"Nonbank visible in {summary['nonbank_visible_pct']}% of AI answers!")
                ...
                for r in results:
                    icon = "✅" if r.get("nonbank_visible") else "❌"
                    with st.expander(f"{icon} {r['prompt'][:80]}"):
                        col1, col2 = st.columns(2)
                        with col1:
                            st.markdown(f"**Nonbank in text:** {'Yes' if r.get('nonbank_in_text') else 'No'}")
                            st.markdown(f"**Nonbank in citations:** {'Yes' if r.get('nonbank_in_citations') else 'No'}")
                ...
                export_rows = [{"prompt": r["prompt"], "nonbank_in_text": r.get("nonbank_in_text"),
                    "nonbank_in_citations": r.get("nonbank_in_citations"),
                    "competitors": ", ".join(r.get("competitors_mentioned", [])),
                    "citations": " | ".join(r.get("citations", [])[:3]),
                } for r in results]
                csv = pd.DataFrame(export_rows).to_csv(index=False)

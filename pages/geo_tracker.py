"""Page 12 — GEO Tracker: discover prompts via Claude → monitor Nonbank mentions in Perplexity → find opportunities."""
...
def page_geo_tracker():
    ...
    st.caption("Discover what prompts users ask AI → Track if Nonbank appears in answers")

    with st.expander("ℹ️ How GEO Tracking works", expanded=False):
        st.markdown("""
**GEO = Generative Engine Optimization** — getting Nonbank mentioned when users ask AI about crypto cards.

**3-step process:**
1. **Discover Prompts** — Claude generates realistic prompts users type into ChatGPT/Perplexity
2. **Monitor** — Each prompt is sent to Perplexity AI. We check if Nonbank appears
3. **Analyze** — See which prompts mention Nonbank, which competitors dominate, where opportunities are
...
""")
    ...
    with tab_monitor:
        ...
        st.markdown("Send each prompt to Perplexity and check if Nonbank appears in the AI answer.")
        ...
        if not prompts:
            ...
            if st.button("▶️ Run Monitor", type="primary"):
                ...
                def on_progress(i, total, result):
                    progress.progress(i / total, text=f"Prompt {i}/{total}: {'✅' if result.nonbank_visible else '❌'} {result.prompt[:60]}...")
    ...
    with tab_results:
        ...
        if not results:
            ...
            col2.metric("Nonbank Visible", f"{summary['nonbank_visible']} ({summary['nonbank_visible_pct']}%)")
            ...
            if opps:
                ...
                st.success("No gaps found — Nonbank appears wherever competitors do!")
            ...
            if selected:
                ...
                if match:
                    ...
                    c1.metric("Nonbank Visible", "✅" if match.nonbank_visible else "❌")

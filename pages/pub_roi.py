...
**Revenue:** total visitors × conversion rate (by language) × LTV (by market+language).
...
""")
    ...
    with st.expander("⚙️ Model Assumptions", expanded=False):
        ...
        with ac1:
            ...
            ref_ctr = st.slider("% of article readers who click to nonbank.io",
                                 0.5, 5.0, 1.5, 0.5,
        ...
        with ac2:
            ...
            default_rank = st.slider("Assumed current rank if unknown", 10, 30, 20,
                                      help="Where nonbank.io ranks for target keyword without this link")
        ...
        with ac3:
            st.markdown("**LTV by language** (starter placeholders — refine once cohort data exists)")
    ...
    with tab_ltv:
        ...
        st.caption("Placeholder values — replace with real cohort LTV once first registrations are measurable.")
        ...
        for lang, ltv in LTV_BY_LANG.items():
            ...
            ltv_rows.append({
                "Language": LANG_LABELS.get(lang, lang.upper()), "LTV/user ($)": ltv,
        ...
        st.info(
            "**How to use this:** LTV × conversion rate × visitors per publication gives you expected "
            "revenue per outlet. As real cohort data comes in, update `LTV_BY_LANG` in "
            "`publication_roi.py` and add high-performing market combos to `LTV_BY_MARKET_LANG`."
        )
        ...
        if ml_rows:
            ...
            st.caption("No market × language overrides yet. Add entries to `LTV_BY_MARKET_LANG` as high-ROI audience segments emerge.")

"""
publication_roi.py — Per-publication ROI model for Nonbank SEO & GEO agent
=======================================================================
Three-layer model:
...
LTVs below are placeholders (Nonbank has no legacy cohort data). Replace with
real values as soon as a first cohort's LTV is measurable.
"""
...
# Placeholder values — Nonbank starts without cohort data. Treat as order-of-
# magnitude estimates only and refine as real data arrives.
...
# ── LTV by market-language combo (override generic lang LTV) ──────────────────
# Empty by default — add market-specific overrides once cohort data exists
# (e.g. ("ARE","en"): 8000 if UAE English cohort consistently over-indexes).
LTV_BY_MARKET_LANG: dict[tuple[str, str], float] = {}

# ── Conversion rates by language ──────────────────────────────────────────────
# Calibrated so: median publication cost / (median_traffic × CTR × CR) ≈ $20–55 CAC
...
def calculate_publication_roi(
    ...
    *,
    ...
    current_rank        : nonbank.io's current ranking for target keyword (default 20)
    article_ctr_pct     : % of outlet's monthly visitors who see the article
    referral_to_site_pct: % of article readers who click through to nonbank.io
